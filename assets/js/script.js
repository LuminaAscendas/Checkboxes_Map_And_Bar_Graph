...
$(document).ready(function(){
	...
	for (var j=1; j <= col; j++) {
		for (i=temp; i < color.length; i++) {
			...
			if(i==row-1 && j==1)break;
			else if(i==row*2-1 && j==2)break;
		}	
		temp=i+1;
	...
	$("#begin_container,#image_container").mouseenter(function(){
		if(!begin_entered) $("#begin_container").attr("title","World Map & Bar Graph");
  		else $("#image_container").attr("title","World Map & Bar Graph");
	});
